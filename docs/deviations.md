...
| C24 | 单实例失败不影响退出码，`run_steps` 的"第一步失败就停"形同虚设 | `run_refinement_on_existing_outputs` 原先在 `failed_count` 非零时只打印警告后 `return`；agent 路径对"产出空 SQL"也不作声 | `src/utils/pipeline.py::run_steps` 的注释写着"继续跑下去会精修一个半成品目录、静默缩小样本，看起来像结果而不是错误"，但这个保护从未被触发过，因为 runner 无论如何都退出 0。**实测事故**：86 实例正式跑中途漏了 `source .env`，臂 A 报 `❌ Failed: 32/86` 之后 **pipeline 照常进入臂 B 并开始 Syncing**；若非人工 Ctrl+C，臂 B 会拿着一个 54/86 的臂 A 跑满 7 小时，最后出一张看似正常的对比表。agent 路径有同型漏洞：产出空 SQL 的实例会被 `_has_agent_output` 判为未完成，进而被 `_sync_instance_dirs` 从两臂剔除（C22），于是样本静默从 86 缩到 85 而无人知晓。<br>**已修**：`run_refinement_on_existing_outputs` 返回未精修实例数，`main()` 据此 `sys.exit(1)`；agent 路径在循环结束后核对本次 `to_run` 的每个实例是否都有产出，缺则列出实例 id 并退出 1。两者都提示"重跑同一条命令即可，已完成的会跳过"。覆盖在 `tests/test_refinement_resume.py::TestExitCode` 与 `TestResume::test_failed_instances_are_reported_to_the_caller` |
| C25 | 降低 refiner 探库轮数会撞上无法满足的探库下限，静默全量报废 | `cte_refiner.py:391` 的 `min_required_sql` 默认 8；`:521-526` 在探库不足时驳回 verdict | 上游两个入口的探库预算本就不一致：`tkboost.sql()` 每片段 5 轮（`turns_per_cte` 下限恒生效，`:603`）且显式传 `min_probes=3`（`:540`）；`sql_agent_runner.py` 每片段 25 轮且**从不传** `min_required_sql`，吃默认 8。想对齐上游只改轮数就会中招：一轮一条探针，5 轮凑不满 8 次，5 轮全耗在 `more probes (n/8)` 的催促上 → 撞上限 → 兜底返回 `{"status": "issues", "issues": ["no_verdict"]}`（`:696`），而该 status 在我们的 runner 里**会触发改写**，等于让 refiner 在毫无诊断的情况下系统性要求重写每个片段。实测更直接：`--refiner-turns 5` 单独传时 `ValueError` 被逐实例的 `except Exception` 吞掉，86 个实例全记为失败而进程退出 0。<br>**已修**：加 `--refiner-turns`（默认 `DEFAULT_REFINER_TURNS = 25`）与 `--refiner-min-probes`（默认 `None`，沿用 `cte_refiner.DEFAULT_MIN_PROBES = 8`），不传时行为与参考轮逐字节一致；`_refiner_options` 在 `min_probes >= turns` 时抛 `ValueError`，`main()` 与 `scripts/run_pipeline.py` 都在跑任何东西之前校验（后者必须自己校验，否则要等 agent 步跑完约 2 小时才报错）。参数只透传给两臂、不给 agent 步。覆盖在 `tests/test_refiner_budget_args.py`（13 例）与 `tests/test_pipeline.py::TestPlanSteps` |
| C23 | refiner 的探库查询没有超时，一条就能无限期卡死整条流水线 | `src/agents/cte_refiner.py:277` 原为裸 `sqlite3.connect(db_path)`，6 个 `cursor.execute` 点都没有时限 | 主 agent 走的 `src/executors/sqlite_executor.py:20` **一直有** 120 秒超时（`threading.Timer` + `conn.interrupt()`），所以这是 refiner 连接上的漏配而非机制缺失。而 refiner 写的是自己的探索性 SQL，会发出 agent 不会发的查询。**实测事故**：86 实例正式跑到第 81 个（`local344`，`f1` 库）时卡死近 4 小时——进程满占一个核、主线程 `R` 态、打开着 `f1.sqlite` 外加三个 `etilqs_*` 排序临时文件、`rchar` 累计读了 **2.4 TB**（84 MB/s 全命中页缓存），日志 4 小时没新增一行，实例目录里零个 refiner 产物。那条 SQL 把 `lap_positions`（613,112 行）自连七次，refiner 在其上生成的探库查询在无索引表上就是笛卡尔爆炸。注意**原始 SQL 本身只要 9.1 秒**，炸的纯粹是探库查询。<br>**已修**：加 `_execute_with_timeout(conn, cursor, sql, fetch_all, seconds)`，6 个 execute 点全部改走它，默认 `QUERY_TIMEOUT_SECONDS = 120.0` 与 agent 侧取同一个值（两套限额会让日志里的超时含义不唯一）。两个设计要点：**时限必须覆盖 fetch**——`execute` 只推进到第一行，只卡住它会把跑飞的投影查询的自旋搬到 `fetchall` 里；**两处 compile 检查用 `fetch_all=False`**，它们只需知道语句能跑，取全量会把廉价检查变成本条要防的那种卡死。超时抛 `TimeoutError`，被 6 个点原有的 `except Exception` 接住并作为探库失败反馈给模型，模型会换一条查询。覆盖在 `tests/test_refiner_query_timeout.py`（7 例，含"普通 SQL 错误不得被报成超时"与"中断后连接仍可用"）；另用真实 `f1.sqlite` 验过：构造的三重自连在预算点被准确中断，连接随后仍可用 |

| C26 | refiner 的 verdict **全部**来自兜底路径，整套自检从未执行过一次 | `cte_refiner.py:622-634` 的接收闸门；`:702-789` 的兜底路径 | 循环内那条 verdict 分支上挂着三项自检（`tests` 非空、LEFT JOIN + CASE 空值、`suggested_fix_sql` 能否编译），失败则把错误追进对话、清空 verdict、`continue` 重出——**机制完整，但一次都没跑过**。实测 E 轮两臂 708 个片段，**708 个（100%）** 走的是"轮数耗尽后强制出 verdict"的兜底路径，该路径只追加一句 `TIME LIMIT REACHED... decide NOW`、调一次 LLM、抠标签、`json.loads` 返回，**不执行 `suggested_fix_sql`、不执行 `tests`、不重试**。于是 107 次 `does not run` 全是在 runner 替换时才第一次被发现。<br>不是探库把轮数耗光了：走兜底那些片段的探库次数**中位数只有 2**。卡住的是闸门的第二个条件 `cte_name_hint and not harness_executed`——`harness_executed` 只在某条探库 SQL 里出现 `FROM <当前CTE名>` 时才置真（`:579`、`:611`），而模型几乎只探基表。原因在措辞：`[Harness tip]` 写的是 `You may compile-test the CTE by ...`，**是"可以"不是"必须"**，且位于 payload 的 95% 处；而 `[MANDATORY_PROBES]` 标着 mandatory，模型老老实实执行。它做了被明确要求的，没做只是"可以"的。<br>**上游完全同构**：`tkboost/__init__.py:574` 导入的就是同一个 `run_refiner`，CTE 阶段同样传 `max_turns=5`、`min_required_sql=3`。所以 **README 报的 +3.6%～+16.9% 是在"每个 verdict 都被催出来、且 `suggested_fix_sql` 未经任何验证"的状态下测出的**。<br>**已修（阶段 7）**：把校验挪到兜底路径**之后**——verdict 到手即把 `suggested_fix_sql` 的第一个 CTE 替换回整条 SQL 执行，跑不通就把错误回传、续用同一对话、重出 verdict，最多 `--verdict-attempts` 次（默认 1，即 E 轮行为）。判据**只有执行**：E 轮 85 条建议里有 17 条结构不合（多 CTE 或改名）却能跑，按结构拒收会损失它们；另有 11 条结构合规却执行失败，说明光看结构也不够。结构发现只用于丰富错误信息（实测 62% 返回多个 CTE、53% 改了名）。`sql_executed` 跨 attempt 不清零，否则探库下限闸门会重新生效、强迫再探 3 次。产物记 `verdict_attempts` / `verdict_validated` / `verdict_validation_errors`。覆盖在 `tests/test_verdict_retry.py`（33 例），并做了活性检查：同一场景下校验器被调用 3 次——阶段 6 把校验挂在循环内时实测 0 次 |
