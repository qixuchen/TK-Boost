...
已跑两轮，只差 refiner 的探库预算，共用**同一份**裸 agent 产出：

| 轮次 | 每片段探库 | 裸 agent | 臂 A（+refiner） | 臂 B（+知识） | refiner 净 | 知识净 | `delta_paper` |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **A. 参考轮** | 25 轮 | 36（41.9%） | **42（48.8%）** | 36（41.9%） | **+6** | **−6** | 0 |
| **B. 对齐上游** | 5 轮 | 36（41.9%） | 37（43.0%） | **32（37.2%）** | **+1** | **−5** | **−4** |

结论两句话：

1. **refiner 的增益全部来自深度探库**（+6 → +1，预算砍到 1/5 就没了）。
2. **知识在两种配置下都有害，量级相近**（−6 / −5）。所以"我们的 refiner 太强、
   规则因冗余而沦为噪声"这个假设**被证伪**（原 §7 第 3 条，详见 §10）。

§1–§9 记录参考轮，**§10 记录对齐上游那轮及它排除掉的假设**。
...
| 只注入 db 作用域规则 | 一个检索过滤条件 | 约 9 小时（一臂） | 是不是 95% 的 generic 规则在淹没信号 | 待跑（首选） |
| 直接采纳 `suggested_fix_sql` | 一段采纳逻辑 | 约 7 小时（一臂，省掉主 agent 重写） | 是不是"剪断的链"损失了知识 | 待跑 |
| 压低每片段注入量至 3–5 条 | 一个截断参数 | 约 9 小时（一臂） | 是规则太多还是规则不对 | 待跑 |
