...
已跑八轮（A–F、H、J；G 并进 H，没有独立的 I 轮），全部共用**同一份**裸 agent 产出（agent 只运行过一次），因此跨轮严格可比：
...
| **F. 候选 SQL 交给 agent** | 5 | 上游 118 条 | agent 重写 + 候选 SQL | 36 | 35（40.7%） | 35 | −1 | **+0** |
| **H. 整条校验—重试** | 5 | 上游 118 条 | **refiner 的 SQL** + 整条校验重试 | 36 | **36（41.9%）** | 35 | ±0 | **−1**† |
| **J. 上下文 FilterKnowledge** | 5 | 上游 118 条 | 同 H + `--context-filter` / `always` | 36 | 36 | 35 | ±0 | **−1**† |

\* C 那轮的 −1 是假象：54/86 个实例**一条规则都收不到**，差值全部来自那一组的运行间随机性；
真正收到规则的 32 个实例是 ±0。详见 §11。

† H / J 的 −1 落在 §11 测得的分数级噪声带内（约 ±1.7 个实例）。H 相对 E：臂 A 从 34 回到 36（refiner 自身破坏被修掉），臂 B 从 37 掉到 35，知识净差 +3 → −1；E 那个 +3 里有 2 分来自对照组被弄坏。J 与 H 总分相同，伤害从「无 db 规则」组挪到「有本库规则仍注入别库 generic」。详见 §16、§17。

**核心结论：知识的正收益只在"直接采纳 refiner 的 SQL"时出现，而那是上游代码的做法，
不是论文的做法。** A–D 轮走的是论文 Alg 5 的路径（反馈交给 agent 重写），知识一律净有害；
E 轮换成上游的直接采纳，知识首次转正（+3），同时 refiner 自身首次转负（−2）。详见 §13。

**而 F 轮否掉了对 E 轮最自然的那个解释。** 它把 refiner 的候选 SQL 放进反馈、保留 agent
整合，信息**确实传到并被部分采信**（相似度 0.23 → 0.45），但知识净只有 **+0**。
所以 E 轮的 +3 不是来自"知识信息传递得更完整"—— **传递不是关键变量**，
差别在 agent 的任务形态（从头组装 vs 直接成为结果）。详见 §14。

**H 轮在 E 的采纳路径上加上整条查询校验与最多 3 次重试。** 臂 A 从 34 回到 36，
E 的 +3 里约 2 分随对照组变干净而消失；知识净变成 −1。详见 §16。

**J 轮给 FilterKnowledge 完整任务上下文，跨库 generic 仍开（`always`）。**
总分与 H 相同，伤害换了分组，知识净仍是 −1。详见 §17。
...
而 E 轮的 `delta_paper`（+1）以及 H / J 的知识净 −1 都在噪声内，轮次之间 1–2 个实例的差异也不能据此排序。
...
§13 直接采纳 refiner 的 SQL，§14 候选 SQL 交给 agent，§15 四实例探针，
**§16 整条校验—重试（H）**，**§17 上下文 FilterKnowledge（J）**。
`--cross-db-generic never` 的 86 实例尚未跑。
...
## 14. 消融 F：把候选 SQL 一并交给 agent（预测不成立）

设计与动机见 [`implementation_plan.md`](./implementation_plan.md) 阶段 5。

### 为什么做

A–D 轮都只把散文式的 `suggested_fix` 转给主 agent，而那个 agent **从未见过任何规则**。
E 轮直接采纳 refiner 的 SQL 拿到了 +3，于是假设：**知识的好处在于那份
knowledge-informed SQL，只要把它传到改写那一步即可**。F 轮把
`suggested_fix_sql` 作为候选 SQL 放进反馈，保留 agent 作为整合者
（Alg 5 第 7 行仍由 agent 产出 `s_t`）。

### 设置与核验

```bash
cp -r outputs/test_agent outputs/cand5_agent
python -u scripts/run_pipeline.py \
  --split data/splits/spider2_sqlite_test_no_reference_leak.txt \
  --out-prefix outputs/cand5 --tkstore tkstore/tkstore_sqlite.csv \
  --include-candidate-sql --refiner-turns 5 --refiner-min-probes 3 \
  --model gpt-4.1 --filter-model gpt-4.1
```

**两臂都要重跑**（渲染方式变了，臂 A 不能复用 `turns5_refonly`）。核验：两臂各 86/86、
日志零失败、目录凭据已写、86 个 `retrieved_rules.json` 的 `include_candidate_sql` 全为
`true` 且 `tkstore.sha1` 为 `3f75f169…`。

### 结果：两个预测都没成立

| 分组 | n | 裸 agent | 臂 A | 臂 B | B−A | 修好 | 改坏 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 总体 | 86 | 36（42%） | **35（41%）** | **35（41%）** | **+0** | 3 | 3 |
| 无 db 规则 | 34 | 13 | 12 | 13 | +1 | 2 | 1 |
| 有 db 规则 | 52 | 23 | 23 | 22 | −1 | 1 | 2 |

阶段 5 的两个可证伪预测是"臂 A 回到 +1"与"知识净 ≥ +3"，实际是 **−1** 与 **+0**。
3 修好 3 改坏全在噪声内（±1/54），`delta_paper` 是 **−1**。
唯一能确定的结论是：**它没有复现 E 轮的 +3。**

被知识改变的 6 个实例：`local018`、`local070`、`local274`、`local283`、`local311`、`local335`。

### 诊断排除了"agent 忽略候选 SQL"

| | 主 agent 改写与 refiner 建议的相似度 |
| --- | --- |
| B 轮（只传散文） | 中位数 **0.23**，平均 0.31，>0.9 的 **1** 个 |
| **F 轮 臂 B** | 中位数 **0.45**，平均 0.48，>0.9 的 **6** 个 |
| **F 轮 臂 A** | 中位数 **0.48**，平均 0.52，>0.9 的 **11** 个 |

**候选 SQL 确实传到了并被部分采信** —— 相似度翻倍、完全照抄的片段从 1 涨到 6–11 个。
阶段 5.2 列的三种失败模式**一个都没完全命中**：既不是盲抄（那该接近 1.0），
也不是忽略（那该还停在 0.23）。问题是"部分采信"没有转化成分数。

### 两个反常现象

**臂 A 的采信度高于臂 B。** 相似度 0.48 vs 0.45、采纳数 74 vs 70 —— 没有知识时 agent
反而更愿意照抄 refiner 的候选 SQL。

**有知识时 refiner 反而更少挑问题。** 判 `issues` 的比例 25% → **22%**，与 A 轮
（36% → 41%）方向相反。加上候选 SQL 之后，"知识让 refiner 更爱挑问题"这个在
A 轮成立的信号消失了。

`no_verdict` 兜底片段在两臂均为 **0**，与前五轮一致。

### 对整体结论的影响

**阶段 5.4 那句"论文架构是对的，上游只是把知识传丢了"作废。** 现有证据更支持：
E 轮那个 +3 **不是**来自"知识信息传递得更完整" —— F 轮把信息传过去了（相似度翻倍）
却没拿到增益。**传递不是关键变量。**

E 轮与 F 轮真正的差别在 **agent 的任务形态**：F 轮的指令仍是"改写 CTE X、输出完整
`<solution>`"，agent 要从头组装整条查询、候选 SQL 只是旁边的参考；E 轮则是 refiner 的
SQL 直接成为结果。这个观察引出阶段 6（G 轮）。

### 八轮总表

| 轮次 | 探库 | store | 修复由谁应用 | 裸 | 臂 A | 臂 B | refiner 净 | 知识净 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| A | 25 | 上游 118 条 | agent 重写 | 36 | **42** | 36 | **+6** | **−6** |
| B | 5 | 上游 118 条 | agent 重写 | 36 | 37 | 32 | +1 | **−5** |
| C | 5 | 上游、仅 db | agent 重写 | 36 | 37 | 36 | +1 | −1\* |
| D | 5 | 我们 24 条 | agent 重写 | 36 | 37 | 34 | +1 | **−3** |
| **E** | 5 | 上游 118 条 | **refiner 的 SQL** | 36 | **34** | **37** | **−2** | **+3** |
| **F** | 5 | 上游 118 条 | agent 重写 + 候选 SQL | 36 | **35** | 35 | −1 | **+0** |
| **H** | 5 | 上游 118 条 | **refiner 的 SQL** + 整条校验重试 | 36 | **36** | 35 | ±0 | **−1**† |
| **J** | 5 | 上游 118 条 | 同 H + `--context-filter` / `always` | 36 | 36 | 35 | ±0 | **−1**† |

\* C 轮的 −1 是样本塌缩造成的假象，见 §11。
† H / J 的 −1 在噪声内；H 相对 E 主要是臂 A 变干净。详见文首大表注与 §16、§17。

---

## 15. 探针实测：阶段 6+7 在 4 个实例上的行为（**不是一轮完整实验**）

86 实例的 H 轮尚未跑。这里记的是阶段 7 实现后在 4 个实例上的行为核验，样本太小不足以谈
分数，但它测出了一个足以改变 H 轮预期的机制事实，所以单独记下。

**实例选取**：从 E 轮"至少有一条 CTE 建议被丢弃"的 32 个实例里取 4 个，覆盖三种失败形态 ——
`local018`（2 CTE，建议改名 + 多 CTE）、`local070`（5 CTE，建议返回 6 个 CTE，结构最糟）、
`local073`（2 CTE，单 CTE 且未改名，纯列契约破裂）、`local167`（5 CTE，混合）。
四者都直接复用 `outputs/adopt5_agent` 的起始 SQL，所以与 E 轮严格配对。

### 15.1 阶段 7 的三条路径各命中一次

17 个片段里重试机制介入了 3 次：

| 片段 | attempts | 拒收次数 | 结局 |
| --- | --- | --- | --- |
| `local018 / category_counts` | 2 | 1（+1 循环内） | 修好并**采纳**，E 轮此处是丢弃 |
| `local073 / base_orders` | 3 | 3 | 三次都没修好，**放弃、保留原 SQL**（§7.3 的兜底） |
| `local073 / exclusions_split` | 3 | 2 | refiner 被拒两次后**改判 ok**、撤回建议 |

第三种是设计里没预料到的：refiner 不修建议，而是缩回去说没问题。对结果无害（E 轮那条建议
本来也被丢弃、保留原 SQL），但说明重试的压力可能把它推向沉默而非推向正确。

`local018` 的完整链路值得记下来，因为它显示**阶段 6 与阶段 7 是耦合生效的**：

| 轮次 | 来源 | CTE 数 | 名字 | 替换进整条 SQL 后 |
| --- | --- | --- | --- | --- |
| attempt 1 | 兜底路径 | 2 | `top_category` | 失败：`no such column: collision_year` |
| attempt 2 / turn 3 | **循环内** | 1 | `category_counts` | 失败：`no such column: speeding_incidents` |
| attempt 2 / turn 4 | **循环内** | 1 | `category_counts` | **通过，采纳** |

attempt 2 的第一个动作是跑 `SELECT * FROM category_counts LIMIT 10` —— 正是 §6.8 查明的、
挡住 708/708 个片段的 `harness_executed` 那道条件。它一置真，**循环内出 verdict 的路第一次通了**，
于是阶段 6 的自检也第一次真正执行，并拦下了 turn 3 那份结构已修对、但还差一个下游列的建议。
**阶段 6 的代码需要阶段 7 先把门打开。**

### 15.2 关键发现：契约破裂的消失主要是"几乎不改了"，不是"改得更对"

为分离"阶段 6 的效果"与"refiner 本身的方差"，用**完全相同的 4 个实例 + E 轮原配置**
（`--model gpt-4.1 --filter-model gpt-4.1 --refiner-turns 5 --refiner-min-probes 3
--adopt-refiner-sql`，逐字对齐 `adopt5_pipeline.log` 里的原命令）又跑了一次：

| | ok（不提建议） | 采纳 | 丢弃（替换后跑不通） |
| --- | --- | --- | --- |
| E 轮原始 | 3/17 | 4 | **10** |
| E 轮复现（同配置、不同次） | 6/17 | 2 | **9** |
| H 轮（阶段 6+7） | **13/17** | 3 | **1** |

两次 E 配置之间，方差能让 3 个片段翻成 ok，但"跑不通"稳在 9–10。H 轮把它压到 1，
这个幅度方差解释不了；ok 从 3–6 跳到 13 也远超方差。

**归因到阶段 6，不是阶段 7**：重试只介入 3 个片段、其中仅 1 个以 ok 收场，
剩下 12 个 ok 出现在重试根本没触发的片段上。所以是 `[DOWNSTREAM]` 可见性 + 段落说明
让 refiner 不再动手。

这给出一个关于论文表述的更具体解释，比 `implementation_plan.md` §6.7 原来写的那句更硬：

> 论文把 `Feedback(q, c_i, K)` 的输入限定为问题 + 单个 CTE，refiner 只能拿**整个问题**
> 去衡量**一个片段**，于是系统性地过度诊断 —— 它抱怨"这个 CTE 没回答用户的问题"，
> 而那本来就不是这个 CTE 的职责，后面的 CTE 会补上。E 配置下它对 14/17 个片段判 issues，
> 其中 9–10 条建议替换回去直接跑不通。给它看下游之后，判 issues 降到 4/17。

### 15.3 对 H 轮的影响：先别急着跑

采纳数从 E 的 2–4 到 H 的 3，**没涨**；涨的是"什么都不做"。知识只能通过 refiner 的编辑
动作影响结果，编辑面从 14/17 压到 4/17，两臂就都趋近裸 agent，知识净差按构造趋近 0。

这正落在 `implementation_plan.md` §7.7 写的止损情形上，但触发原因与当时设想的不同 ——
不是"E 轮那个 +3 不稳定"，而是**阶段 6 把编辑面几乎清零了**。

分数层面这 4 个实例看不出差别，样本也太小：E 复现 3/4、H 轮 3/4
（`local018`/`local070`/`local167` 对，`local073` 两边都错）。

**候选下一步**（未决）：把阶段 6 拆成两个独立开关（`[DOWNSTREAM]` 可见性 vs 校验器），
定位到底是哪一个让 refiner 变哑。若是可见性，可以只留校验器 + 重试，保住编辑面。

> ### ⚠️ 本节的核心推断已被 §16 的 86 实例数据推翻
>
> "编辑面被清零"是 4 实例上的小样本假象。全量实测 ok 率只从 73% 升到 82%（不是 4 实例上
> 看到的 18–35% → 76%），而**采纳数反而上升**（臂 A 45 → 59，臂 B 58 → 68）。
> 下面"知识将无处作用"的预测不成立，H 轮的 −1 有另外的成因，见 §16.3。
>
> 仍然成立的部分：跑不通的建议大幅减少（本节 9–10/17 → 1/17，全量 49 → 5），
> 以及 `local274` 两轮一致被知识改坏。

### 15.4 产物位置

| 目录 | 配置 |
| --- | --- |
| `outputs/h_probe_tk/`（`local018`） | 阶段 6+7，`--verdict-attempts 3` |
| `outputs/h_probe2_tk/`（其余 3 个） | 同上 |
| `outputs/e_replay_tk/`（4 个） | E 轮原配置，方差基线 |

注意 `h_probe_tk/local018` 是在"循环内拒收也记进审计"那个修复**之前**跑的，
所以它的 `verdict_validation_errors` 只有 1 条（漏了循环内那次）；
另两个目录是修复之后跑的。

---

## 16. 轮次 H：verdict 校验—重试环（阶段 6+7，86 实例）

第一轮把"知识增益"与"refiner 自身破坏"分离开的实验。配置见
`implementation_plan.md` §7.8：`--refiner-turns 5 --refiner-min-probes 3
--adopt-refiner-sql --validate-fix-in-context --verdict-attempts 3`，
两臂**直接复用 `outputs/adopt5_agent`**（E 轮那 86 份 `execution_query.sql`），
所以 H 与 E 不仅两臂配对，逐实例的起始 SQL 也完全相同。

完整性：两臂都 86/86、`❌ Failed: 0`。臂 A 1.58 小时（1.1 分钟/实例），
臂 B 3.09 小时（2.2 分钟/实例），与 §7.8 预估的 2 / 3 小时相符。

### 16.1 主结果：知识增益从 +3 变成 −1，而这主要是对照组变干净了

| | 裸 agent | 臂 A（refiner 无知识） | 臂 B（refiner + 知识） | 知识净差 |
| --- | --- | --- | --- | --- |
| E 轮 | 36 | **34**（−2） | 37（+1） | **+3** |
| H 轮 | 36 | **36**（±0） | 35（−1） | **−1** |

**E 轮那个 +3 里有 2 分来自臂 A 被 refiner 弄坏，而不是臂 B 被知识改好。**
知识相对裸 agent 的净贡献在 E 轮本来就只有 +1。阶段 6+7 修掉了臂 A 的破坏（−2 → 0），
那 2 分随之消失；剩下的变化是臂 B 从 +1 掉到 −1。

±1～3 都落在 §11 测得的 ±2%（≈±1.7 个实例）噪声带内，所以定性结论是
**看不出知识增益**，而不是"知识有害到可测"。但下面的分组和逐条分析给出了比总分更强的信号。

### 16.2 按"有无 db 规则可用"分组，harm 完全集中在一侧

| | n | 裸 | 臂 A | 臂 B | 知识净差 |
| --- | --- | --- | --- | --- | --- |
| H 轮 · **无 db 规则** | 34 | 13 | 13 | **11** | **−2** |
| H 轮 · 有 db 规则 | 52 | 23 | 23 | 24 | **+1** |
| E 轮 · 无 db 规则 | 34 | 13 | 12 | 13 | +1 |
| E 轮 · 有 db 规则 | 52 | 23 | 22 | 24 | +2 |

**H 轮全部的损伤都发生在那 34 个"所在库在 store 里一条 db 规则都没有"的实例上**，
它们能收到的 100% 是别库的 generic 规则。有 db 规则的 52 个仍是 +1。
E 轮看不到这个分裂，因为臂 A 被破坏污染了两组的基线。

### 16.3 机制：编辑面没有塌缩，是"有害但能跑"的编辑开始落地了

354 个片段的状态分布：

| | ok（不提建议） | 采纳 | 丢弃（替换后跑不通） |
| --- | --- | --- | --- |
| E 轮臂 A | 260 (73%) | 45 | **49 (14%)** |
| H 轮臂 A | 290 (82%) | **59** | **5 (1%)** |
| E 轮臂 B | 237 (67%) | 58 | **59 (17%)** |
| H 轮臂 B | 282 (80%) | **68** | **4 (1%)** |

阶段 6+7 的直接目标达成了：跑不通的建议从 49／59 压到 5／4。但**采纳数同时上升**
（45→59、58→68），有采纳的实例从 36→44（臂 A）、47→48（臂 B）。所以 §15 那个
"编辑面被清零"的推断是小样本假象。

真正的因果是另一条：

> E 轮那 59 条跑不通的建议里有相当一部分是**有害**建议，被"执行不通过"这道粗糙闸门
> 意外拦住了。我们把闸门修好，过滤作用也一并去掉，于是"语法正确但语义有害"的编辑开始落地。

量化：**有采纳编辑的实例**相对裸 agent 的净效应，臂 B 从 E 轮的 **+1**（修好 2、弄坏 1）
变成 H 轮的 **−1**（修好 2、弄坏 3）。采纳率上去了，采纳质量没有。

重试机制本身工作正常（臂 A 数据）：`verdict_attempts` 分布 `{1: 328, 2: 17, 3: 9}`，
介入 26 个片段 → 修好并采纳 **12**、三次放弃 **5**、refiner 自行撤回判 ok **9**。
那 12 条若无阶段 7 会落进"丢弃"。

### 16.4 逐条读七处翻转

知识造成的翻转共 7 处（臂 A → 臂 B）。**先剔除不可归因的两条**：`local049` 和 `local070`
的 `rules_used` 是空的 —— 没有任何带规则的片段被采纳，所以它们的改好来自两臂各自独立跑
refiner 的随机差异，不是知识效应。这也提醒逐实例翻转不能直接当成知识效应读。

**改坏 4 条：**

| 实例 | 库（db 规则数） | 片段 | 发生了什么 |
| --- | --- | --- | --- |
| `local061` | `complex_oracle`（0） | `growth_and_projected` | 臂 A 判 ok。臂 B 用规则 26 判定 INNER JOIN 漏掉"只在单年出现的 prod_id/month"，改成 `UNION` 键 + 双 `LEFT JOIN` + `COALESCE` 补 0。题目要的正是 INNER JOIN 语义 |
| `local330` | `log`（0） | `normalized_activity` | 臂 A 判 ok。臂 B 担心路径归一化产生重复，给事件日志的归一化 CTE 加了 `SELECT DISTINCT` —— 一去重事件计数就变了，而题目问的就是事件数 |
| `local302` | `bank_sales_trading`（12） | `percent_changes` | **两臂都改了，方向不同**。臂 A 把 LEFT JOIN 拼装换成条件聚合（`SUM(CASE WHEN week_date BETWEEN ...)`），对了；臂 B 保留 LEFT JOIN 结构，转而去加"过滤分组列的 NULL/空值"（规则 46，来自 `modern_data`）。知识把它从正确修法**引开** |
| `local274` | `oracle_sql`（0） | final SELECT | **E 轮与 H 轮都被知识改坏**，唯一重复信号。两臂诊断一致（"没有时间/批次列，FIFO 无法实现"）却都动手：臂 A 改完等价于原样，臂 B 把 `AVG(pl.qty)` 改成 `SUM(pl.qty)` 并改了列名。gold 要的是平均值 |

`local061`、`local330`、`local274` 用上的规则**没有一条属于本库** ——
分别来自 `California_Traffic_Collision`、`SQLITE_SAKILA`、`bank_sales_trading`、`music`。

**改好 1 条（可归因）：** `local310`（`f1`，0 条 db 规则，18 条规则）。两臂都改了 final SELECT，
臂 A 的诊断纠缠在"只准用 `results` 表却引用了 `races`"，臂 B 抓住了真问题
（"按年聚合的 join key 用错，拿 year 当 race_id"）并改对。

### 16.5 归因链：generic 规则为什么能跨库生效

检索的保留判据是"**scope 是 generic** 或者 **库名匹配**"，两者取或
（`tkstore/tagger_index.py:566-572`）：
...
`is_generic` 一为真就短路，`db_matches` 算了也不用。而 store 里
**118 条规则每一条的 `db` 列都是具体库名**：66 条 `scope='db'`、52 条 `scope='generic'`，
**generic 里 `db='all'` 的有 0 条**。也就是说"这条规则来自哪个库"这个信息完整存在，
只是对那 52 条从不被查阅。

`scope` 的来源：populate 时 LLM 把从**单个**失败实例学到的东西分两段输出，
`DATABASE_MEMORIES` → `scope='db'`，`GENERIC_MEMORIES` → `scope='generic'`
（`tkstore/builder.py:181-191` 切分，`:49` 写 scope）。所以"generic"是 LLM 只看着一个库时
做的一次主观判断，落库后被当作普适事实使用，**此后没有任何机制检验它在别的库是否成立**。

这个跨库迁移本身**符合论文**（Retrieve 的立意就是通用规则跨库复用），已登记的偏离是另两条：
B2（`table`/`column` 两维不参与过滤）与 B6（generic 且只命中一个常见操作就丢弃，
这是代码自加的降噪启发式）。真正没有对应物的是"generic 标签的可靠性从未被校验"。

### 16.6 产物位置

| 目录 | 内容 |
| --- | --- |
| `outputs/adopt5_agent/` | 共享 agent 产出（E、H 两轮共用） |
| `outputs/h_retry_refonly/` | 臂 A |
| `outputs/h_retry_tk/` | 臂 B |
| `outputs/h_retry_comparison.csv` | 逐实例对比表 |
