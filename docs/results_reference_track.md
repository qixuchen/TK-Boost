...
已跑三轮，共用**同一份**裸 agent 产出（agent 只运行过一次）：

| 轮次 | 每片段探库 | 规则范围 | 裸 agent | 臂 A（+refiner） | 臂 B（+知识） | refiner 净 | 知识净 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **A. 参考轮** | 25 轮 | all | 36（41.9%） | **42（48.8%）** | 36（41.9%） | **+6** | **−6** |
| **B. 对齐上游** | 5 轮 | all | 36 | 37（43.0%） | **32（37.2%）** | **+1** | **−5** |
| **C. 只用 db 规则** | 5 轮 | db | 36 | 37（复用 B 的臂 A） | 36 | — | −1\* |

\* 这个 −1 是假象：54/86 个实例在 db-only 下**一条规则都收不到**，总体差值全部来自那一组的
运行间随机性；真正收到规则的 32 个实例是 **±0**。详见 §11。

结论三句话：

1. **refiner 的增益全部来自深度探库**（+6 → +1，预算砍到 1/5 就没了）。
2. **知识在两种探库配置下都有害，量级相近**（−6 / −5）。所以"我们的 refiner 太强、
   规则因冗余而沦为噪声"这个假设**被证伪**（原 §7 第 3 条，详见 §10）。
3. **只留 db 规则没有消除伤害**，它只是让 63% 的实例不再接受任何知识；
   在真正收到规则的 32 个实例上，带 generic 的配置反而略好（详见 §11）。

**噪声底线已量出**：§11 用 54 个零注入实例做了天然重复实验 —— SQL 层面 63% 的实例会写出
不同结果，但**分数层面只有 2%（1/54）发生变化**。所以 −6 / −5 是真实效应，
而幅度 1–2 个实例的差异都在噪声内。

§1–§9 记录参考轮，§10 记录对齐上游那轮，§11 记录 db-only 那轮与噪声测量。
...
| 直接采纳 `suggested_fix_sql` | 一段采纳逻辑 | 约 7 小时（一臂，省掉主 agent 重写） | 是不是"剪断的链"损失了知识 | 待跑 |
| 压低每片段注入量至论文的 **4.2 条** | 一个截断参数 | 约 3 小时（一臂） | 是规则太多还是规则不对 | 待跑（首选） |
...
- [ ] 消融：**只注入 db 作用域规则**（首选，直接检验一号假设，约 9 小时／一臂）
- [ ] 消融：直接采纳 `suggested_fix_sql`（检验二号假设，约 7 小时／一臂）
