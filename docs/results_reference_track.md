...
§13 直接采纳 refiner 的 SQL，**§14 候选 SQL 交给 agent**。

**下一步（阶段 6 / G 轮，尚未实现）**：E 轮拦下的 107 条建议里 33 条是"输出契约被破坏"——
refiner 只看得见自己和**前面**的 CTE（`sql_agent_runner.py:693-696` 取 `ctes[:idx_cte]`），
改掉输出列名就打挂下游。而 refiner 内部**已有**自检重试循环
（`cte_refiner.py:570-590`），只是它检的是孤立片段而非替换回整条后的查询。
计划见 [`implementation_plan.md`](./implementation_plan.md) 阶段 6。
